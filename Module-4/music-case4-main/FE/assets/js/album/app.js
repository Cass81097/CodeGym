...
async function listAlbumMusic(id) {
    try {
        const res = await axios.get(`http://localhost:3000/songs/id/?idAlbum=${id}`);
        let data = res.data;
        let str = `
            <tr>
                <th>#</th>
        ...
        data.map((item, i) => {
            str += `
                <tr class="songItem">
                    ...
                    <td class="song">
                        <i class="bi playListPlay bi-play-circle-fill" id="${item.id}" onclick="listenMusic(${item.id})"></i>
                    </td>
                </tr>
            `;
